...
| Stakeholder portal API `GET /api/v1/portal/projects` (2026-05-09 probe) | **FAIL deploy** — HTML **404** on staging + production (portal routes missing on Worker bundle or stale deploy) |
...
4. **Stakeholder live finance sanity:** After redeploy, confirm `GET /api/v1/portal/projects` returns **200 JSON** (not HTML 404), then run `bash scripts/verify/stakeholder_finance_sanity.sh` with `STAKEHOLDER_SMOKE_*` → update `FINAL_CUSTOMER_FINANCE_ISOLATION_AUDIT.md`.  
