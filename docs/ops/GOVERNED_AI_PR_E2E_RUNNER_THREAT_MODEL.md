...
| Bypass token in logs/artifacts | Header-only bypass; stdout/stderr captured to ephemeral files; raw harness output encrypted (AES-256-GCM, staging-derived key) and transferred via run-scoped cache; redacted artifact only uploaded from Job 3 |
