...
| C-04 | Supabase migration readiness was outside automated deploy contour | `scripts/release/check-env-config.sh` (`migrations` mode), deploy workflows | Schema drift risk between application deploy and DB state | Release Eng + DB | in_progress (latest staging/prod runs `26410298260` / `26410422613` passed, but migration preflight check step is still `skipped`; requires `SUPABASE_ACCESS_TOKEN` + `SUPABASE_PROJECT_REF` secrets to close) |
