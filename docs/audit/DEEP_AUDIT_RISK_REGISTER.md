...
| C-04 | Supabase migration readiness was outside automated deploy contour | `scripts/release/check-env-config.sh` (`migrations` mode), deploy workflows | Schema drift risk between application deploy and DB state | Release Eng + DB | in_progress (migrations-preflight jobs added and run conditionally when required secrets are configured; awaiting pipeline evidence with configured secrets) |
...
| H-04 | Cron/job triggers are weak if `REQUIRE_CRON_SECRET` disabled | `apps/web/lib/api/cron-auth.ts` | Unauthorized job triggering / abuse risk | Platform | in_progress (production defaults to require secret + tests added + fail-fast deploy guard added; pending operator action: configure GitHub `CRON_SECRET` secret for production rollout) |
