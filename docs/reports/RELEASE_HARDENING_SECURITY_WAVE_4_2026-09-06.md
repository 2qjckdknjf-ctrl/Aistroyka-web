...
Wave 4 is stacked on the corrected Wave 3 exact head. The repository `CI Check` workflow currently triggers only for pull requests whose base is `main`; stacked PRs therefore do not automatically receive that workflow. This is a validation limitation, not a PASS.

A main-based validation PR is used only to run the exact combined stack through CI. Earlier combined validation exposed (1) a generated-schema typing issue in the trusted estimate writer and then (2) stale tests that still assumed the pre-fail-closed writer setup. Wave 3 was corrected without weakening runtime authorization, and Wave 4 was rebuilt on that corrected head before revalidation.

## Required gates

- clean Wave 4-only diff against Wave 3;
