...
- **Staging workflow hardening (repo):** `deploy-cloudflare-staging.yml` now supports `workflow_dispatch` with `ref` input and uses patched deploy flow aligned with production: `wrangler deploy --env staging --dry-run --outdir .open-next/deploy` → `node scripts/patch-bundle-require.cjs` → `wrangler deploy --env staging --no-bundle --config wrangler.deploy.toml`.
- **Cloud check (2026-04-07, updated):** staging runtime incident is mitigated. Route `staging.aistroyka.ai/*` now points to existing Worker `hiair`, which reverse-proxies to `https://www.aistroyka.ai`. Current smoke checks: `/api/v1/health` => 200, `/ru` => 200, `/ru/login` => 200, unauthenticated `/api/v1/projects` => 401 (expected). This is a temporary topology; final clean state remains direct deploy to `aistroyka-web-staging` + route cutover.
