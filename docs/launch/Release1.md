...
- **Latest staging validation (2026-04-07):** deploy runs `24106627305`, `24106960458`, `24107209947` are green; v1 compatibility routes for `/api/v1/auth/login` and `/api/v1/analysis/process` are published; `AI_ANALYSIS_URL` wired in deploy config; processing executes. `OPENAI_API_KEY` is set in runtime, but provider calls still fail due upstream OpenAI quota (`429`), so analysis currently ends as `failed` (`502 All AI providers failed or are unavailable`).
