...
- **Latest staging validation (2026-04-07):** deploy runs `24106627305`, `24106960458`, `24107209947` are green; v1 compatibility routes for `/api/v1/auth/login` and `/api/v1/analysis/process` are published; `AI_ANALYSIS_URL` wired in deploy config; processing executes. Remaining external blocker is vision provider secret (`OPENAI_API_KEY` or another supported provider key) — otherwise analysis completes as `failed` with provider `503`.
