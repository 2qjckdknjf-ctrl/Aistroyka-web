...
**Task chat device UI (related, not a substitute for C4–C12):** see `TASK_CHAT_DEVICE_UI_SMOKE_REPORT.md` — TEXT / DELETE / AUTHORIZATION / CROSS_TENANT **PASS**; PHOTO/VOICE/VIDEO UI **BLOCKED** (device Locked); OFFLINE media **REFUSED_BY_DESIGN**; `size_bytes` **PASS** on staging+prod `a401693` for image/voice; E2E **NO**; TestFlight **NO**.
