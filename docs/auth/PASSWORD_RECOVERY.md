...
4. User follows the email link. Callback exchanges `code` for a session. When `recovery=1`, it redirects to the sanitized `callback` path (default `/{locale}/reset-password`) and **does not** run Apple/Google identity linking or onboarding.
...
Merge (do not overwrite) the Auth allow-list with `apps/web/scripts/set-supabase-auth-urls.mjs` so iOS custom schemes stay present.
