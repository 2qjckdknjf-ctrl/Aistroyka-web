...
| GET | `/api/v1/ai/requests` | Yes | List tenant AI jobs (`ai_analyze_media`, `ai_analyze_report`). See below. |
| GET | `/api/v1/ai/requests/:id` | Yes | Single AI job detail for the current tenant. Also returns `vision_configured`. |

Contracts: `AnalyzeImageRequestSchema`, `AnalysisResultSchema`, `AnalyzeImageErrorSchema`.

### List AI requests

Query: `limit` (default 50, max 100), `offset`, `status`, `from`, `to`, `q`.

- Date-only `from` / `to` (`YYYY-MM-DD`) expand to inclusive UTC day bounds
  (`T00:00:00.000Z` … `T23:59:59.999Z`) so same-day jobs are not truncated at midnight.
- Response includes:
  - `data` — sanitized job rows (secrets stripped)
  - `total` — filtered count (or client-side filter size when `q` is set)
  - `summary` — **unfiltered** counts `{ total, queued, running, success, failed, dead }` so UIs can distinguish “never created” vs filtered empty / failed backlog
  - `vision_configured` — whether any vision provider key is present in this runtime env

Async media jobs resolve images through `resolve-ai-media-image.ts` with a tenant/project
path guard. Operator recovery brief: [`ai/P0_AI_PIPELINE_RECOVERY.md`](./ai/P0_AI_PIPELINE_RECOVERY.md).
...
On create, the service enqueues push type `task_message` to assignees (and
in-app manager notifications when the sender is a field worker). See
[`PUSH-NOTIFICATIONS.md`](./PUSH-NOTIFICATIONS.md) and
[`runbooks/PUSH_DELIVERY.md`](./runbooks/PUSH_DELIVERY.md).
