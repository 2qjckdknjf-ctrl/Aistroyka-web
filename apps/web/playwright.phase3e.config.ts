...
/**
 * Phase 3E — required multi-role E2E proof.
 * Run only after `node tests/phase3e/preflight.mjs` exits 0.
 * No soft-skips; no screenshot baselines; mutation-guarded.
 */
...
function loadRootServiceRole(): Record<string, string> {
  const rootEnv = path.join(__dirname, "../../.env.local");
  ...
  for (const line of readFileSync(rootEnv, "utf8").split("\n")) {
    ...
    return v.includes(".") ? { SUPABASE_SERVICE_ROLE_KEY: v } : {};
  }
  return {};
}
    ...
    process.env.CI || process.env.PLAYWRIGHT_SKIP_WEB_SERVER
      ...
      : {
          ...
          env: {
            ...process.env,
            ...publicEnv,
            ...(process.env.SUPABASE_SERVICE_ROLE_KEY ? {} : loadRootServiceRole()),
          },
        },
});
