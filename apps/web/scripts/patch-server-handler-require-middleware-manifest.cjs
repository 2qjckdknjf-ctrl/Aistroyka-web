...
const originalMarker = "(function(x){if(typeof require<\"u\")return require.apply(this,arguments);throw Error('Dynamic require of \"'+x+'\" is not supported')})";
const alreadyPatchedMarker = "(function(x){if(typeof x===\"string\"&&x.includes(\"middleware-manifest.json\"))return{version:3,middleware:{},functions:{},sortedMiddleware:[]};if(typeof require<\"u\")return require.apply(this,arguments);throw Error('Dynamic require of \"'+x+'\" is not supported')})";
const replacement = "(function(x){if(typeof x===\"string\"&&(x.includes(\"middleware-manifest\")||x.includes(\"middleware_manifest\")))return{version:3,middleware:{},functions:{},sortedMiddleware:[]};if(typeof require<\"u\")return require.apply(this,arguments);throw Error('Dynamic require of \"'+x+'\" is not supported')})";

const marker = code.includes(alreadyPatchedMarker) ? alreadyPatchedMarker : originalMarker;
if (!code.includes(marker)) {
  console.warn("patch-server-handler-require-middleware-manifest: marker not found (format may have changed), skip");
  process.exit(0);
}
