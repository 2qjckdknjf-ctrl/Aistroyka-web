...
describe("change-orders.policy", () => {
  it("delegates manage checks to the project-scoped client-request cohort", async () => {
    vi.mocked(clientRequestsPolicy.canManageClientRequests).mockResolvedValue(false);
    const denied = await canManageChangeOrders({} as never, {
      tenantId: "t1",
      userId: "worker-1",
      role: "member",
    ...
    const allowed = await canManageChangeOrders({} as never, {
      tenantId: "t1",
      userId: "mgr-1",
      role: "member",
      subscriptionTier: "free",
