...
describe("canManageClientRequests", () => {
  ...
  it("denies when project is not in caller tenant", async () => {
    vi.mocked(projectRepo.getById).mockResolvedValue(null);
    const ok = await canManageClientRequests({} as never, ctx("owner") as never, "foreign-project");
  ...
  it("allows tenant owner when project belongs to tenant", async () => {
    ...
    const ok = await canManageClientRequests({} as never, ctx("owner") as never, "p1");
    expect(ok).toBe(true);
  });
  ...
  it("denies tenant member without project manager/owner membership", async () => {
    ...
    vi.mocked(membersRepo.getMembership).mockResolvedValue({ role: "worker" } as never);
    const ok = await canManageClientRequests({} as never, ctx("member") as never, "p1");
    expect(ok).toBe(false);
  });
  ...
  it("allows project manager membership", async () => {
    ...
    vi.mocked(membersRepo.getMembership).mockResolvedValue({ role: "manager" } as never);
    const ok = await canManageClientRequests({} as never, ctx("member") as never, "p1");
    expect(ok).toBe(true);
  });
  ...
  it("allows project owner membership", async () => {
    ...
    vi.mocked(membersRepo.getMembership).mockResolvedValue({ role: "owner" } as never);
    const ok = await canManageClientRequests({} as never, ctx("member") as never, "p1");
    expect(ok).toBe(true);
  });
});
