...
function buildStaticSections(): RomaQaCenterSection[] {
  return [
    section({
      ...
      status: "coming_soon",
      maturity: "planned",
      sourceAvailability: "Not available — no audit run store yet",
      description: "Safe, owner-gated audit runs across product surfaces. V1 is read-only planning only.",
      currentCapability: "Document audit types and prerequisites. No run trigger, no CI coupling.",
      futureCapability:
        "Scoped audit jobs (web smoke, API contract, RBAC matrix) with immutable reports and explicit owner approval gates.",
      blockers: ["Audit execution pipeline not implemented", "No run history store", "No safe-audit approval workflow"],
      subAreas: [
        { id: "preflight", label: "Release preflight", status: "coming_soon", note: "Health + deploy truth + grant checks" },
        { id: "rbac", label: "RBAC boundary", status: "coming_soon", note: "Platform vs tenant isolation" },
    ...
    section({
      ...
      status: "coming_soon",
      maturity: "planned",
      sourceAvailability: "Not available — no run history store",
      description: "Previous runs, trends, known recurring issues.",
      currentCapability: "No historical runs — V1 does not record audit executions.",
      futureCapability: "Immutable run log, trend charts, recurring issue fingerprints.",
      blockers: ["No audit run persistence", "Execution disabled"],
    }),
    section({
      id: "reports",
      title: "Reports",
