...
const MIGRATION = resolve(
  __dirname,
  "../../supabase/migrations/20260807120000_harden_viewer_project_jobs_estimate_writes.sql"
);

describe("viewer / jobs / estimate RLS hardening migration", () => {
  const sql = readFileSync(MIGRATION, "utf8");

  it("excludes viewers from project writes and restricts delete to owner/admin", () => {
  ...
  it("requires writer + tenant match for document and milestone writes", () => {
    expect(sql).toMatch(
  ...
  it("scopes customer_estimates writes to manage cohort and blocks forged decisions", () => {
    expect(sql).toMatch(
