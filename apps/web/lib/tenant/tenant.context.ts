 ...
 *
 * Active tenant (T-P2-1 / 2C_active_tenant_selection):
 * - Prefer validated `x-tenant-id` header, then `aistroyka_active_tenant` cookie
 * - Else owned tenant, else deterministic membership order
 * - Explicit unauthorized/invalid claims fail closed (absent context)
 */
...
const CLIENT_VALUES: ClientProfile[] = [
  "web",
  "ios_full",
  "ios_lite",
  "ios_worker",
 ...
 * then explicit active tenant (header/cookie) or owned/membership fallback.
...
export async function getTenantContextFromRequest(request: Request): Promise<TenantContextOrAbsent> {
  ...
  // Handler-level lite allow-list (CF Workers may bypass Next middleware for /api/v1/*).
  // Fail closed before auth/tenant resolution or any repository access.
  const url = new URL(request.url);
  if (checkLiteAllowList(url.pathname, request.method, request.headers.get("x-client"))) {
    return {
      tenantId: null,
      userId: null,
      role: null,
  ...
  const active = await resolveActiveTenantId(supabase, user.id, request);
  if (!active.tenantId) {
    return { tenantId: null, userId: user.id, role: null, subscriptionTier: null, clientProfile, traceId };
  }

  const role = await getRoleInTenant(supabase, active.tenantId, user.id);
  if (!role) {
    return { tenantId: null, userId: user.id, role: null, subscriptionTier: null, clientProfile, traceId };
  }
  ...
  try {
    const [permissionSet, scopes] = await Promise.all([
      ...
      getUserScopes(supabase, active.tenantId, user.id),
    ]);
...
async function getRoleInTenant(
