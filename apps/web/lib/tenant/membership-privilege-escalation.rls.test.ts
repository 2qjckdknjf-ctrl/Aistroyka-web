...
const migration = readFileSync(
  join(
    __dirname,
    "../../supabase/migrations/20260906090000_release_hardening_membership_privileges.sql"
  ),
  "utf8"
);

describe("release hardening membership privilege-escalation RLS", () => {
  ...
  it("installs a BEFORE UPDATE trigger that blocks authenticated role escalation", () => {
  ...
  it("splits project_stakeholders FOR ALL and blocks invitee INSERT", () => {
  ...
  it("revokes privileged project_stakeholders column updates from authenticated", () => {
    ...
    expect(migration).not.toMatch(
      /grant update \([^)]*stakeholder_role[^)]*\) on public\.project_stakeholders/
    );
    expect(migration).not.toMatch(
      /grant update \([^)]*project_id[^)]*\) on public\.project_stakeholders/
    );
    expect(migration).not.toMatch(
      /grant update \([^)]*tenant_id[^)]*\) on public\.project_stakeholders/
    );
  });
});
