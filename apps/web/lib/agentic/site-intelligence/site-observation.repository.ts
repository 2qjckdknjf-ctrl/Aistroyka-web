...
/**
 * Reads durable image-analysis results from the existing `ai_analysis` source of truth.
 *
 * Tenant/project scope is established by selecting canonical project media first.
 * The latest result is then fetched independently for each scoped media object in
 * bounded batches, so a long analysis history for one media cannot crowd other media
 * out of a global result cap.
 *
 * `media.uploaded_at` is the media provenance time. Analysis creation time is never
 * substituted for media capture/upload provenance. Malformed persisted timestamps
 * fail closed rather than falling back to the current wall clock.
 */
