...
export function resolveAgentIntent(message: string): AgentIntent {
  ...
  if (/угрожа|сдач|срок|deadline|delay|handover|delivery/.test(m)) return "delivery_threat";
  ...
  if (/площадк|что видно|по фото|по видео|визуальн|site status|site progress|visual progress|photo evidence|video evidence/.test(m)) return "site_status";
  return "general";
}
