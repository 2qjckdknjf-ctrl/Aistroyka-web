...
export function TablePagination({
  ...
  return (
    <nav
      ...
      aria-label={ariaLabel ?? tDetail("tablePagination")}
    >
      <div className="flex items-center gap-3 text-[var(--aistroyka-font-caption)] text-aistroyka-text-secondary">
        <span className="tabular-nums">
          {start}–{end} {tDetail("of")} {totalCount}
        </span>
        {onPageSizeChange && (
          <select
            aria-label={tDetail("rowsPerPage")}
            value={pageSize}
