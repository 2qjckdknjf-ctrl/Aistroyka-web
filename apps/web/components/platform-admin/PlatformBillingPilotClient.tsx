...
export function PlatformBillingPilotClient({
  initialWorkspaces,
}: {
  ...
  async function fetchDiagnostics(workspaceId: string) {
    ...
    setDiagnostics(null);
    try {
      const res = await fetch(`/api/v1/platform/billing/workspace-status?workspaceId=${encodeURIComponent(workspaceId)}`);
  ...
  async function reprocessWorkspaceEvents(workspaceId: string) {
    ...
    try {
      const res = await fetch("/api/v1/platform/billing/reprocess-workspace-events", {
        method: "POST",
  ...
  async function addToPilot(workspaceId: string) {
    ...
    setActionResult(null);
    try {
      const res = await fetch("/api/v1/platform/billing/pilot-workspaces", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workspaceId }),
      });
  ...
  async function removeFromPilot(workspaceId: string) {
    ...
    setActionResult(null);
    try {
      const res = await fetch(`/api/v1/platform/billing/pilot-workspaces/${workspaceId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? tDetail("failed"));
