...
export function PlatformAdminConsoleClient() {
  ...
  async function loadAll() {
    const [overviewRes, tenantsRes, usersRes, ticketsRes, auditRes, diagnosticsRes] = await Promise.all([
      ...
      fetch("/api/v1/platform/support/tickets"),
      fetch("/api/v1/platform/audit?limit=30"),
      fetch("/api/v1/platform/diagnostics"),
    ]);
    ...
    if (
      ...
      setError("Failed to load platform admin data.");
      return;
    }
  ...
  async function loadThread(ticketId: string) {
    const response = await fetch(`/api/v1/platform/support/tickets/${ticketId}/messages`);
  ...
  async function sendReply(e: React.FormEvent) {
    ...
    const response = await fetch(`/api/v1/platform/support/tickets/${selectedTicketId}/messages`, {
      method: "POST",
  ...
  async function updateStatus(ticketId: string, status: string) {
    const response = await fetch("/api/v1/platform/support/tickets", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-aistroyka-title2 font-semibold tracking-tight">Platform admin overview</h1>
        <p className="mt-2 text-aistroyka-subheadline text-aistroyka-text-secondary">
          Isolated platform admin surface for cross-tenant operations, support, diagnostics, and audit.
        </p>
      </header>
