...
describe("GET/POST /api/v1/worker/issues", () => {
  ...
  it("binds a validated evidence session when the worker attaches a photo", async () => {
    const res = await POST(
      new Request("https://test/api/v1/worker/issues", {
        method: "POST",
  ...
  it("rejects an invalid evidence session before creating the issue", async () => {
    validateIssueEvidenceSession.mockResolvedValueOnce({
      ok: false,
      error: "Evidence session not owned by user",
      status: 403,
    });
    const res = await POST(
      new Request("https://test/api/v1/worker/issues", {
        method: "POST",
        headers: { "content-type": "application/json", "x-idempotency-key": "k-bad-photo" },
        body: JSON.stringify({
          project_id: "proj-1",
          title: "Unsecured fence",
          evidence_upload_session_id: "sess-other-user",
        }),
      })
    );
    expect(res.status).toBe(403);
    expect(createWorkerReportedIssue).not.toHaveBeenCalled();
  });
});
