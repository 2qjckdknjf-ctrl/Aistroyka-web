...
vi.mock("@/lib/domain/portal/portal.service", () => ({
  getPortalProjectDecisions: vi.fn(),
}));

describe("GET /api/v1/portal/projects/:id/decisions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("returns 500 when payload contains forbidden finance keys", async () => {
    vi.mocked(portalService.getPortalProjectDecisions).mockResolvedValue({
      data: {
        project: { id: "p1", name: "Tower" },
        decisions: [],
        client_requests: [{ id: "r1", title: "Approve", budget_pressure: "high" }],
        capabilities: {},
      } as never,
      error: "",
    });

    const res = await GET(new Request("https://test/api/v1/portal/projects/p1/decisions"), {
