/**
 * GET /api/v1/portal/projects/:id/proof — proof packs are distributed via share links today.
 */
...
import { getPortalProjectProof } from "@/lib/domain/portal/portal.service";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await getPortalProjectProof(supabase, ctx, projectId);
  ...
  if (!safety.ok) {
    console.error(
      `Blocked customer-finance leak in /api/v1/portal/projects/${projectId}/proof: ${safety.path ?? safety.key}`
    );
