/**
 * GET /api/v1/portal/projects/:id/progress
 */
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await getPortalProjectProgress(supabase, ctx, projectId);
  ...
  if (!safety.ok) {
    console.error(
      `Blocked customer-finance leak in /api/v1/portal/projects/${projectId}/progress: ${safety.path ?? safety.key}`
    );
