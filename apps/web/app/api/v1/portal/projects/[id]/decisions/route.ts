/**
 * GET /api/v1/portal/projects/:id/decisions — document review queue + structured client requests.
 */
...
import { getPortalProjectDecisions } from "@/lib/domain/portal/portal.service";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await getPortalProjectDecisions(supabase, ctx, projectId);
  ...
  if (!safety.ok) {
    console.error(
      `Blocked customer-finance leak in /api/v1/portal/projects/${projectId}/decisions: ${safety.path ?? safety.key}`
    );
