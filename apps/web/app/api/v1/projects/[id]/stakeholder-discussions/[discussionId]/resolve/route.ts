/**
 * POST /api/v1/projects/:id/stakeholder-discussions/:discussionId/resolve
 */
...
import { resolveDiscussion } from "@/lib/domain/stakeholder-discussions/stakeholder-discussions.service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string; discussionId: string }> }) {
  ...
  const summary = typeof body.resolution_summary === "string" ? body.resolution_summary : "";
  const { ok, error } = await resolveDiscussion(supabase, ctx, projectId, discussionId, summary);
  if (error === "Insufficient rights") return NextResponse.json({ error }, { status: 403 });
  if (!ok) return NextResponse.json({ error: error || "Resolve failed" }, { status: 400 });
