...
const ERROR_CODE_CRON_TICK = "cron_tick_error";
...
export async function POST(request: Request) {
  ...
  if (!admin) {
    return NextResponse.json({ ok: false, error: "Admin client not configured", error_code: "admin_not_configured" }, { status: 503 });
  }
  ...
  try {
    ...
    if (tenantsError) {
      return NextResponse.json(
        { ok: false, error: "Failed to list tenants", error_code: ERROR_CODE_CRON_TICK },
        { status: 503 }
      );
    }
    ...
    for (const t of tenantList) {
      try {
        ...
        // per-tenant skip; continue with others
      }
    }
    ...
    return NextResponse.json(
      { ok: false, error: "Cron tick failed", error_code: ERROR_CODE_CRON_TICK },
      { status: 503 }
    );
  }
}
