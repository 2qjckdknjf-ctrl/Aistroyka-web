...
vi.mock("@/lib/platform/billing-readiness/billing-pilot-resolution.service", () => ({ getBillingPilotDiagnostics: (...a: unknown[]) => mockGetBillingPilotDiagnostics(...a) }));
...
describe("app/api/v1/platform/billing/pilot-status/route.test.ts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequirePlatformOwnerApi.mockResolvedValue(ownerOk("OWNER"));
    mockGetAdminClient.mockReturnValue({}); mockGetBillingPilotDiagnostics.mockResolvedValue({});
  });

  describe("GET /api/v1/platform/billing/pilot-status", () => {
    it("calls requirePlatformOwnerApi first with mode read", async () => {
      ...
      const res = await GET(requestFor("/api/v1/platform/billing/pilot-status", "GET"));
