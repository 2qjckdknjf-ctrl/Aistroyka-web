...
vi.mock("@/lib/platform/billing-readiness/billing-pilot-ops.service", () => ({ reprocessBillingEventOps: (...a: unknown[]) => mockReprocessBillingEventOps(...a) }));
...
describe("app/api/v1/platform/billing/reprocess-event/route.test.ts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequirePlatformOwnerApi.mockResolvedValue(ownerOk("OWNER"));
    mockGetAdminClient.mockReturnValue({}); mockReprocessBillingEventOps.mockResolvedValue({ ok: true });
  });

  describe("POST /api/v1/platform/billing/reprocess-event", () => {
    it("calls requirePlatformOwnerApi first with mode write", async () => {
      ...
      const res = await POST(requestFor("/api/v1/platform/billing/reprocess-event", "POST", { body: {"eventId":"evt-1"} }));
