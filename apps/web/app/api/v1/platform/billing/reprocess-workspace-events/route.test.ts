...
vi.mock("@/lib/platform/billing-readiness/billing-pilot-ops.service", () => ({ reprocessBillingEventsForWorkspaceOps: (...a: unknown[]) => mockReprocessBillingEventsForWorkspaceOps(...a) }));
...
describe("app/api/v1/platform/billing/reprocess-workspace-events/route.test.ts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST /api/v1/platform/billing/reprocess-workspace-events", () => {
    it("calls requirePlatformOwnerApi first with mode write", async () => {
      ...
      const res = await POST(requestFor("/api/v1/platform/billing/reprocess-workspace-events", "POST", { body: {"workspaceId":"ws-1"} }));
