...
describe("POST /api/analysis/process (legacy)", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  async function post(headers: Record<string, string> = {}) {
    ...
    return POST(
      new Request("http://test/api/analysis/process", {
        method: "POST",
        headers: { "x-request-id": "req-legacy-1", ...headers },
      })
    );
  }
  ...
  it("returns 403 for stakeholder", async () => {
    ...
    const res = await post();
    expect(res.status).toBe(403);
    expect(mockAuthorize).toHaveBeenCalledWith(expect.anything(), "analysis:trigger");
  });
  ...
  it.each(["member", "admin", "owner"] as const)("allows %s and passes exact tenantId", async (role) => {
    mockGetTenantContext.mockResolvedValue(baseCtx({ role, tenantId: "tenant-exact" }));
    mockAuthorize.mockReturnValue(true);
    const res = await post();
    ...
    expect(mockProcessOneJob).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ tenantId: "tenant-exact", traceId: "req-legacy-1" })
    );
    expect(res.headers.get("Deprecation")).toBe("true");
  });
  ...
  it("returns 429 when rate limited", async () => {
    mockCheckRateLimit.mockResolvedValue({ limited: true, message: "Tenant rate limit exceeded. Try again later." });
    const res = await post();
    expect(res.status).toBe(429);
    expect(mockProcessOneJob).not.toHaveBeenCalled();
  });

  it("continues with tenant-scoped process when rate-limit storage throws", async () => {
  ...
  it("returns sanitized 500 on processing error", async () => {
    mockProcessOneJob.mockResolvedValue({
      ok: false,
      reason: "error",
      message: "secret other-tenant job uuid leak",
    });
    const res = await post();
    expect(res.status).toBe(500);
    const data = (await res.json()) as { error?: string };
    expect(data.error).toBe("Processing failed");
    expect(data.error).not.toContain("other-tenant");
  });

  it("preserves legacy deprecation headers on success", async () => {
    const res = await post();
    expect(res.status).toBe(200);
    expect(res.headers.get("Deprecation")).toBe("true");
