...
export function ClientPortalDefectsListClient({ projectId }: { projectId: string }) {
    ...
    return (
      <Card>
        <EmptyState
          ...
          title="Could not load punch list"
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <h1 className="text-aistroyka-title3 font-semibold">Punch list</h1>
        <p className="mt-1 text-sm text-aistroyka-text-secondary">
          Report snags or defects for your project team. This is not a general bug tracker.
        </p>
        <Button type="button" size="sm" variant="secondary" className="mt-3" onClick={() => setOpen((o) => !o)}>
          {open ? "Cancel" : "Report an item"}
        </Button>
        {open ? (
          <div className="mt-4 space-y-2 rounded border border-aistroyka-border-subtle p-3">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              This may block completion until fixed
            </label>
            ...
            <li key={r.id}>
              ...
              >
                ...
                <span className="flex flex-wrap items-center gap-2">
                  {r.is_blocking ? <Badge className={blockingBadgeClass}>Blocking</Badge> : null}
        ...
        {rows.length === 0 ? <p className="mt-3 text-sm text-aistroyka-text-secondary">No items yet.</p> : null}
      </Card>
    </div>
  );
}
