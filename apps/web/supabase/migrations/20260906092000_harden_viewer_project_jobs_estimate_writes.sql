...
-- projects: viewers cannot CUD; delete requires tenant owner/admin.
...
-- project_documents / project_milestones: writer + tenant/project consistency.
...
create policy project_documents_write_internal_insert on public.project_documents
  for insert to authenticated
  with check (public.is_internal_tenant_writer_for_tenant(tenant_id) and public.project_belongs_to_tenant(project_id, tenant_id));
...
-- customer_estimates: project manager cohort only.
...
create policy customer_estimates_insert_internal on public.customer_estimates
  for insert to authenticated
  with check (public.can_manage_project_membership(tenant_id, project_id) and public.project_belongs_to_tenant(project_id, tenant_id));
...
create policy customer_estimate_items_update_internal on public.customer_estimate_items
  ...
  using (exists (
    select 1 from public.customer_estimates e
    where e.id = estimate_id and public.can_manage_project_membership(e.tenant_id, e.project_id)
  ))
  with check (exists (
    select 1 from public.customer_estimates e
    where e.id = estimate_id
...
create trigger customer_estimates_enforce_decision_service
  ...
  for each row execute function public.enforce_customer_estimate_decision_via_service();

revoke all on function public.enforce_customer_estimate_decision_via_service() from public;

-- jobs/job_events: split broad authenticated policies and guard lifecycle updates.
...
create trigger jobs_enforce_authenticated_update
  ...
  for each row execute function public.enforce_jobs_authenticated_update_guard();
