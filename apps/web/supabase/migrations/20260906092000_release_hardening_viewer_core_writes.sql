-- RELEASE HARDENING: close direct PostgREST write holes where read cohorts
-- (including role=viewer) were reused for high-blast-radius mutation policies.
--
-- Scope:
-- - projects / project_documents / project_milestones
-- - customer estimates + decision integrity
-- - jobs / job_events lifecycle integrity
--
-- App APIs already gate most of these operations; this migration closes the
-- direct authenticated Supabase REST bypass.
...
comment on function public.is_tenant_owner_or_admin(uuid) is
  'True for tenant owner or admin, including tenants.user_id owner.';
...
-- ---------------------------------------------------------------------------
-- customer_estimates: project manage cohort only + block forged approval via REST
-- ---------------------------------------------------------------------------
...
-- Decision outcomes must go through authorized server paths, not direct PostgREST.
...
comment on function public.enforce_customer_estimate_decision_via_service() is
  'Blocks direct-REST forgery of customer estimate approval/rejection; service_role only.';

-- ---------------------------------------------------------------------------
-- jobs / job_events: authenticated may SELECT + INSERT enqueue;
-- UPDATE only to free dedupe_key on terminal rows; DELETE/lifecycle transitions server-only.
-- ---------------------------------------------------------------------------
...
comment on function public.enforce_jobs_authenticated_update_guard() is
  'Blocks direct-REST job status/payload forgery; allows clearing dedupe_key on terminal jobs only.';
...
drop policy if exists job_events_select_internal on public.job_events;
drop policy if exists job_events_insert_internal on public.job_events;
