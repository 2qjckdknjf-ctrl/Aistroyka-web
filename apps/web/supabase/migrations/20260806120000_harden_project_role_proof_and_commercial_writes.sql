-- CRITICAL: close PostgREST privilege holes on project_members, proof_pack_shares,
-- and commercial write surfaces that trusted any internal tenant reader.
--
-- 1) project_members UPDATE allowed managers to set role='owner' (self-promote).
-- 2) proof_pack_shares FOR ALL exposed bearer tokens to viewers and allowed mutation.
-- 3) project_change_orders / commercial items writes allowed any internal reader
--    (including field workers / viewers) via direct Supabase REST.
-- 4) Inserts did not require projects.tenant_id = row.tenant_id (cross-tenant FK skew).

-- ---------------------------------------------------------------------------
-- project_members: block authenticated role/tenant/project/user escalation
-- ---------------------------------------------------------------------------
...
comment on function public.enforce_project_members_privilege_change() is
  'Blocks PostgREST self-promotion of project_members.role (e.g. manager→owner); tenant owner/admin may change roles.';

-- Managers may still insert/update non-role fields and invite workers, but cannot
-- choose an elevated role on INSERT either.
...
-- (live names from phase2_customer_portal_commercial_rls)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Defects: keep portal open-defect insert; require tenant match on all inserts;
-- internal update/delete require manage cohort.
-- ---------------------------------------------------------------------------
