-- RELEASE HARDENING: close direct-REST privilege holes on project membership,
-- proof-pack bearer shares, change orders, commercial items and defects.
--
-- App-layer checks are necessary but insufficient because authenticated Supabase
-- clients can address public tables through PostgREST. These policies mirror the
-- project manager/owner cohort and enforce tenant/project consistency.

-- ---------------------------------------------------------------------------
-- project_members: block authenticated role/identity escalation
-- ---------------------------------------------------------------------------
...
comment on function public.enforce_project_members_privilege_change() is
  'Blocks direct-REST self-promotion of project_members.role; tenant owner/admin may change roles.';

-- Managers may invite worker/contractor/manager roles. Elevated owner role requires
-- tenant owner/admin authority.
...
comment on function public.project_belongs_to_tenant(uuid, uuid) is
  'True when projects.id belongs to the supplied tenant_id; blocks cross-tenant FK skew.';
...
-- ---------------------------------------------------------------------------
-- Change orders + events: project manage cohort + tenant match
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Commercial items: same project manager cohort + tenant match
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Defects: keep constrained portal open-defect insert; management mutations require
-- project manage cohort. Always require tenant/project consistency.
-- ---------------------------------------------------------------------------
