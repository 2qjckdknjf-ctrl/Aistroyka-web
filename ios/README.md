...
- **CI (manual):** `.github/workflows/ios-e2e-integration.yml` — `workflow_dispatch` runs API chain + Layer B UITests on `macos-latest` when `PILOT_E2E_*` and production Supabase secrets are configured. Operator notes: `.github/workflows/README.md` (includes known phantom `push` failure on merge — ignore; run manually).
